...
FOR UPDATE USING (
  id = public.org_id() AND public.org_role() IN ('admin', 'owner')
);
...
FOR ALL USING (
  org_id = public.org_id() AND public.org_role() IN ('admin', 'owner', 'member')
);
...
FOR ALL USING (
  org_id = public.org_id() AND public.org_role() IN ('admin', 'owner')
);
